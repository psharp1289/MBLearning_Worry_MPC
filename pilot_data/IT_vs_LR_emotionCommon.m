...
inv_temp=[1,2,5,8]; %inverse temperatures
trials_learning=[1,3,6,9,12,15,18,21,24];
associability=[1.2,1.5,2.0,3.0];
trials_testing=20;
counter=0;
    ...
    for counter=1:9 %consider a number of learning trials
        for c=1:100
            ...
            for j=1:trials_learning(counter)
                if rand<0.6;
                    obs=1;
                    value=value+((lr*associability(3))*(obs-value));
                else
                    obs=0;
...
title('Varying Inv Temp while LR=0.1, Associability=2, Emotion=COMMON')
subplot(4,1,2);
plot(X,lr_mid(1,1:9),X,lr_mid(2,1:9),X,lr_mid(3,1:9),X,lr_mid(4,1:9));
ylim([6,21]);
legend('temp=1','temp=2.0','temp=3.0','temp=4.0');
title('Varying Inv Temp while LR=0.25, Associability=2, Emotion=COMMON')
subplot(4,1,3);
plot(X,lr_mat(1,1:9),X,lr_mat(2,1:9),X,lr_mat(3,1:9),X,lr_mat(4,1:9));
ylim([6,21]);
legend('lr=0.1','lr=0.2','lr=0.25','lr=0.3');
title('Varying LR while Inv Temp=1, Associability=2, Emotion=COMMON')
subplot(4,1,4);
plot(X,lr_mat2(1,1:9),X,lr_mat2(2,1:9),X,lr_mat2(3,1:9),X,lr_mat2(4,1:9));
ylim([6,21]);
legend('lr=0.1','lr=0.2','lr=0.25','lr=0.3');
title('Varying LR while Inv Temp=5, Associability=2, Emotion=COMMON')
